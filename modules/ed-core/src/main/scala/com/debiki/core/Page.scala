...
object TopicLayout {
  ...
  /** Top level replies sorted by time, and each such reply has its own threaded
    ...
    * (If separate *topics* for each status update, is too heavy weight.)  */
  ...
  object ProgressWithFlatReplies extends TopicLayout(1006)

  /** Like the above, but best replies first (instead of by time). Facebook uses this. */
  object DiscussionWithFlatReplies extends TopicLayout(1005)
  */

  // Maybe not.
  //object SplitDiscussionWithFlatRepliesAndProgressBelow extends TopicLayout(1006)
